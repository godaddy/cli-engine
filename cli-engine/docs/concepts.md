...
is rendered. The topic's registered names are declared as clap possible values, so
`<bin> guide <TAB>` completes real topics and an unrecognized topic is rejected by clap itself
(exit code 2) rather than reaching a command-specific error.
